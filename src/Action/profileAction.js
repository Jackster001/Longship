...
export const sendFriendRequest = (requested) => async (dispatch)=>{
    try{
        const responseMessage = await axios.post(`${dev}/api/profile/send_request`, requested);
        dispatch({
            type: "SEND_REQUEST",
...
export const acceptFriendRequest = (currentID, requesterID) => async (dispatch)=>{
    try{
        ...
        const responseMessage = await axios.post(`${dev}/api/profile/accept_request`, {currentID, requesterID}, {headers:{'Authorization':localStorage.jwtToken}});
        return {
            type: "ACCEPT_REQUEST",
            payload: responseMessage
        }
...
export const openMessageBox = (currentID, friendID) => async (dispatch)=>{
    try{
        const responseMessage = await axios.post(`${dev}/api/profile/open_chat`, {currentID, friendID}, {headers:{'Authorization':localStorage.jwtToken}});
